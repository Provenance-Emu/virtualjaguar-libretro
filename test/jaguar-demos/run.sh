...
select_roms_smoke() {
    ...
    while IFS= read -r line || [ -n "${line}" ]; do
        ...
        if [ ! -f "${VENDOR_DIR}/${line}" ]; then
            echo "WARN: smoke ROM missing: ${line}" >&2
            continue
        fi
        echo "${line}"
    done < "${SMOKE_LIST}"
...
run_suite() {
    ...
    if [ ! -f "${BASELINE}" ]; then
        echo "WARN: no BASELINE.txt yet; writing one from this run" >&2
        write_baseline_from_results "${results_file}"
        rm -f "${roms_file}"
        return 0
    fi
    ...
    if [ "${mode}" = smoke ]; then
        ...
        if [ ! -s "${gate_base}" ]; then
            echo "WARN: none of the smoke ROMs are in BASELINE.txt yet" >&2
            echo "      writing baseline rows from this smoke run" >&2
            write_baseline_from_results "${results_file}"
            rm -f "${roms_file}"
            return 0
        fi
...
case "${1:-}" in
    ...
    smoke|full)
        ...
        shift
        cmd_fetch
        run_suite "${mode}" "$(find_core "${1:-}")"
        ;;
    baseline)
        shift
        cmd_fetch
        JAGUAR_DEMOS_WRITE_BASELINE=1 run_suite full "$(find_core "${1:-}")"
        ;;
    *)
        usage
        ;;
